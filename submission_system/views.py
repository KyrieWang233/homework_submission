...
from submission_system.models import Homework, Courser, HomeworkAnswer, Comment
...
def submission_detail(request, submission_id):
    submission = HomeworkAnswer.objects.get(pk=submission_id)
    if HomeworkAnswer.objects.get(pk=submission_id).creator != request.user and not request.user.is_superuser:
        raise Http404
    if request.method == 'POST':
        comment = Comment(content=request.POST['content'],comment=HomeworkAnswer.objects.get(pk=submission_id))
        comment.save()
